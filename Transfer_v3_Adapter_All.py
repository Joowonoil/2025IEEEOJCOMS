...
class AdapterTransferLearningEngine:
    """
    Adapter Transfer Learning Engine (v3)
    Parameter-efficient transfer learning using Adapter modules
    """
    ...
    def load_model(self):
        """Load model with Adapter modules"""
        # Create a temporary config file for this scenario
        temp_conf = copy.deepcopy(self._conf)
        temp_conf_path = Path(__file__).parents[0].resolve() / 'config' / f'temp_{self._scenario_name}_adapter.yaml'
        with open(temp_conf_path, 'w', encoding='utf-8') as f:
            yaml.dump(temp_conf, f)

        # Initialize Estimator_v3 (handles Adapter modules automatically)
        self._estimator = Estimator_v3(f'temp_{self._scenario_name}_adapter.yaml').to(self._device)

        print(f"\n[{self._scenario_name}] Adapter applied to Estimator_v3.")
    ...
    def set_optimizer(self):
        """Set optimizer for trainable parameters (Adapter weights)"""
    ...
    def train(self):
        ...
        print(f"\n{'='*60}")
        print(f"Starting Adapter Transfer Learning: {self._scenario_name}")
        print(f"{'='*60}\n")
        ...
        print(f"\n{'='*60}")
        print(f"Completed Adapter Transfer Learning: {self._scenario_name}")
        print(f"{'='*60}\n")
    ...
    def save_model(self, iteration):
        ...
        # Save entire estimator (includes adapter weights)
...
def main():
    ...
    config_file = 'config_transfer_v3_adapter_all.yaml'
    scenarios = ['InH', 'InF', 'UMi', 'UMa', 'RMa']

    print("\n" + "="*70)
    print("Adapter Transfer Learning (v3) - All Scenarios")
    print("="*70)
    ...
    for idx, scenario in enumerate(scenarios, 1):
        ...
        try:
            engine = AdapterTransferLearningEngine(config_file, scenario)
            engine.load_model()
            engine.train()
