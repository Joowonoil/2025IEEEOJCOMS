...
class LoRATransferLearningEngine:
    """
    LoRA Transfer Learning Engine
    Parameter-efficient transfer learning using LoRA (Low-Rank Adaptation)
    """
    ...
    def load_model(self):
        ...
        temp_conf_path = Path(__file__).parents[0].resolve() / 'config' / f'temp_{self._scenario_name}_lora.yaml'
        with open(temp_conf_path, 'w', encoding='utf-8') as f:
            yaml.dump(temp_conf, f)

        # Initialize Estimator_v4
        self._estimator = Estimator_v4(f'temp_{self._scenario_name}_lora.yaml').to(self._device)
        ...
        if peft_config_dict:
            ...
            if peft_type == 'LORA':
                ...
                print(f"\n[{self._scenario_name}] LoRA applied to Estimator_v4.")
    ...
    def set_optimizer(self):
        """Set optimizer for trainable parameters (LoRA weights)"""
    ...
    def train(self):
        ...
        print(f"\n{'='*60}")
        print(f"Starting LoRA Transfer Learning: {self._scenario_name}")
        print(f"{'='*60}\n")
        ...
        print(f"\n{'='*60}")
        print(f"Completed LoRA Transfer Learning: {self._scenario_name}")
        print(f"{'='*60}\n")
...
def main():
    ...
    config_file = 'config_transfer_v4_lora_all.yaml'
    scenarios = ['InH', 'InF', 'UMi', 'UMa', 'RMa']

    print("\n" + "="*70)
    print("LoRA Transfer Learning - All Scenarios")
    print("="*70)
    ...
    for idx, scenario in enumerate(scenarios, 1):
        ...
        try:
            engine = LoRATransferLearningEngine(config_file, scenario)
            engine.load_model()
            engine.train()
