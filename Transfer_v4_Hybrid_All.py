...
class HybridTransferLearningEngine:
    """
    Hybrid Transfer Learning Engine (Prompt + LoRA)
    Combines Prompt Learning and LoRA for parameter-efficient transfer learning
    """
    ...
    def load_model(self):
        """Load model and apply both Prompt Learning and LoRA"""
        # Create a temporary config file for this scenario
        temp_conf = copy.deepcopy(self._conf)
        temp_conf_path = Path(__file__).parents[0].resolve() / 'config' / f'temp_{self._scenario_name}_hybrid.yaml'
        with open(temp_conf_path, 'w', encoding='utf-8') as f:
            yaml.dump(temp_conf, f)

        # Initialize PromptEstimator_v4 (handles Prompt Learning)
        self._estimator = PromptEstimator_v4(f'temp_{self._scenario_name}_hybrid.yaml').to(self._device)
        ...
        if peft_config_dict:
            ...
            if peft_type == 'LORA':
                lora_config = LoraConfig(
                    ...
                    target_modules=peft_config_dict.get('target_modules', ["mha_q_proj", "mha_v_proj", "ffnn_linear1"]),
                ...
                print(f"\n[{self._scenario_name}] Hybrid PEFT (Prompt + LoRA) applied to Estimator_v4.")
    ...
    def set_optimizer(self):
        """Set optimizer for trainable parameters (Prompt tokens + LoRA weights)"""
    ...
    def train(self):
        ...
        print(f"\n{'='*60}")
        print(f"Starting Hybrid Transfer Learning: {self._scenario_name}")
        print(f"{'='*60}\n")
        ...
        print(f"\n{'='*60}")
        print(f"Completed Hybrid Transfer Learning: {self._scenario_name}")
        print(f"{'='*60}\n")
...
def main():
    ...
    config_file = 'config_transfer_v4_hybrid_all.yaml'
    scenarios = ['InH', 'InF', 'UMi', 'UMa', 'RMa']

    print("\n" + "="*70)
    print("Hybrid Transfer Learning (Prompt + LoRA) - All Scenarios")
    print("="*70)
    ...
    for idx, scenario in enumerate(scenarios, 1):
        ...
        try:
            engine = HybridTransferLearningEngine(config_file, scenario)
            engine.load_model()
            engine.train()
