"""
LoRA Pareto Frontier Experiment
Tests multiple LoRA ranks across all scenarios to create parameter-performance curve
Ranks: 4, 8, 16, 20
Scenarios: InH, InF, UMi, UMa, RMa
Total: 4 ranks × 5 scenarios = 20 runs
"""
...
class LoRAParetoEngine:
    def __init__(self, conf_file, scenario_name, rank_config_name):
        ...
        rank_conf = base_conf['lora_configs'][rank_config_name]
        ...
        self._conf['training']['saved_model_name'] = f'Large_estimator_v4_to_{scenario_name}_lora_{rank_config_name}'

        # Update LoRA configuration
        self._conf['ch_estimation']['peft']['r'] = rank_conf['r']
        self._conf['ch_estimation']['peft']['lora_alpha'] = rank_conf['lora_alpha']

        self._scenario_name = scenario_name
        self._rank_config_name = rank_config_name
        ...
        if self._use_wandb:
            wandb.init(
                ...
                name=f"{scenario_name}_LoRA_{rank_config_name}",
                config=self._conf,
                reinit=True
            )
    ...
    def load_model(self):
        ...
        temp_conf_path = Path(__file__).parents[0].resolve() / 'config' / f'temp_{self._scenario_name}_{self._rank_config_name}.yaml'
        with open(temp_conf_path, 'w', encoding='utf-8') as f:
            yaml.dump(temp_conf, f)

        self._estimator = Estimator_v4(f'temp_{self._scenario_name}_{self._rank_config_name}.yaml').to(self._device)

        peft_config_dict = self._conf['ch_estimation'].get('peft', {})
        if peft_config_dict:
            lora_config = LoraConfig(
                r=peft_config_dict.get('r', 20),
            ...
            print(f"\n[{self._scenario_name}-{self._rank_config_name}] LoRA applied.")
    ...
    def train(self):
        ...
        print(f"\n{'='*70}")
        print(f"LoRA Pareto: {self._scenario_name} - {self._rank_config_name}")
        print(f"{'='*70}\n")
        ...
        for it, data in enumerate(self._dataloader):
            ...
            if (it + 1) % logging_step == 0:
                ...
                print(f"[{self._scenario_name}-{self._rank_config_name}] Iter {it + 1}/{num_iter} | "
...
def main():
    config_file = 'config_pareto_lora.yaml'
    scenarios = ['RMa', 'UMa', 'UMi', 'InF', 'InH']  # REVERSED
    rank_configs = ['r20', 'r16', 'r8', 'r4']  # REVERSED

    print("\n" + "="*70)
    print("LoRA Pareto Frontier Experiment")
    print("="*70)
    print(f"Ranks: {', '.join(rank_configs)}")
    print(f"Scenarios: {', '.join(scenarios)}")
    print(f"Total runs: {len(rank_configs)} × {len(scenarios)} = {len(rank_configs) * len(scenarios)}")
    print("="*70 + "\n")

    for scenario_idx, scenario in enumerate(scenarios, 1):
        for rank_idx, rank_config in enumerate(rank_configs, 1):
            run_num = (scenario_idx - 1) * len(rank_configs) + rank_idx
            total_runs = len(scenarios) * len(rank_configs)

            print(f"\n{'#'*70}")
            print(f"# Run {run_num}/{total_runs}: {scenario} - {rank_config}")
            print(f"{'#'*70}\n")

            try:
                engine = LoRAParetoEngine(config_file, scenario, rank_config)
                engine.load_model()
                engine.train()
                ...
                print(f"\nError in {scenario}-{rank_config}: {e}")
                import traceback
                traceback.print_exc()
                continue

    print("\n" + "="*70)
    print("LoRA Pareto Frontier Experiment Completed!")
    print("="*70 + "\n")
