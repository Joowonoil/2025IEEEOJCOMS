...
class PromptTransferLearningEngine: # 프롬프트 기반 전이 학습 엔진 클래스 정의
    ...
    def train(self): # 훈련 메서드
        ...
        for it, data in enumerate(self._dataloader): # 훈련 데이터로더 순회 (이터레이션 번호와 데이터 가져오기)
            ...
            try:
                ...
                torch.nn.utils.clip_grad_norm_(self._estimator.parameters(), max_norm=self._conf['training']['max_norm']) # 그래디언트 클리핑 (학습 가능한 모든 파라미터에 적용)
        ...
        self.save_prompt_model(self._conf['training'].get('saved_model_name', 'final_model')) # 최종 Prompt 모델 저장
    ...
    def save_prompt_model(self, file_name): # Prompt 모델 저장 메서드
        try:
...
if __name__ == "__main__": # InF 환경 특화 전이학습 실행
    #torch.autograd.set_detect_anomaly(True) # 자동 미분 이상 감지 활성화 (주석 처리됨)
    
    # InF 환경 전용 설정 파일 사용
    conf_file = 'config_transfer_v4_prompt_InF.yaml' # InF 환경 특화 Prompt 설정 파일 사용
    
    print("=" * 60)
    print("InF (Indoor Factory) Prompt Transfer Learning Start")
    print("=" * 60)
    print(f"Config file: {conf_file}")
    print(f"Target environment: Indoor Factory (Los/Nlos)")
    ...
    print("InF prompt transfer learning starting...")
    engine.train() # InF 데이터로 Prompt 전이학습 수행
    
    print("=" * 60)
    print("InF Prompt transfer learning completed!")
    ...
    try:
        final_model_name = engine._conf['training'].get('saved_model_name', 'Large_estimator_v4_to_InF_prompt')
