...
class ChannelDataset(IterableDataset):
    def __init__(self, params):
        # load channel pdp data WITH scenario metadata tracking
        self._ch_pdp_list = []
        self._ch_pdp_scenarios = []

        for ch_type in params['channel_type']:
            file_path = Path(__file__).parents[0].resolve() / 'dataset' / 'PDP_processed' / ('PDP_' + ch_type + '.mat')
            pdp_data = scipy.io.loadmat(file_path)['pdp'][0, :]
            self._ch_pdp_list.append(pdp_data)

            # Extract scenario prefix (e.g., "InF_Los_50000" -> "InF_Los")
            # Keep Los/Nlos distinction for more granular control
            scenario = '_'.join(ch_type.split('_')[:2])  # Get "InF_Los", "RMa_Nlos", etc.
            self._ch_pdp_scenarios.append(scenario)

        # Create index-to-scenario mapping for O(1) lookup
        self._pdp_idx_to_scenario = []
        for i, pdp_data in enumerate(self._ch_pdp_list):
            scenario = self._ch_pdp_scenarios[i]
            self._pdp_idx_to_scenario.extend([scenario] * len(pdp_data))
        self._pdp_idx_to_scenario = np.array(self._pdp_idx_to_scenario, dtype=object)

        # Concatenate all PDP data
        self._ch_pdp = np.concatenate(self._ch_pdp_list)
        ...
        # Load scenario-specific distance ranges
        self._distance_ranges = params['distance_ranges']  # dict: {"InF_Los": [min, max], ...}
    ...
    def __next__(self):
        # Sample PDP indices first
        ch_pdp_size = self._ch_pdp.size
        ch_pdp_idx = self._rng.integers(low=0, high=ch_pdp_size, size=(self._batch_size,))

        # Sample distances based on the scenario of each selected PDP
        dist = np.zeros((self._batch_size, 1))
        for i, pdp_idx in enumerate(ch_pdp_idx):
            scenario = self._pdp_idx_to_scenario[pdp_idx]
            if scenario in self._distance_ranges:
                dist_range = self._distance_ranges[scenario]
                dist[i] = self._rng.uniform(low=dist_range[0], high=dist_range[1])
            else:
                raise ValueError(f"Distance range for scenario '{scenario}' not found in distance_ranges config")
