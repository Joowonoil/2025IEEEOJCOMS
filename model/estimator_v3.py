...
class Estimator_v3(nn.Module): # Estimator_v3 클래스 정의
    def __init__(self, conf_file):
        ...
        # Adapter 설정 확인 - enabled=False이면 Adapter 비활성화
        adapter_config = self._conf['ch_estimation'].get('adapter', {})
        adapter_enabled = adapter_config.get('enabled', True)  # 기본값은 True (기존 동작 유지)
        
        if adapter_enabled:
            # Adapter 활성화
            adapter_bottleneck_dim = adapter_config['bottleneck_dim']
            adapter_dropout = adapter_config['dropout']
            print(f"EstimatorV3: Using Adapter (bottleneck_dim={adapter_bottleneck_dim})")
        else:
            # Adapter 비활성화 - None 전달
            adapter_bottleneck_dim = None
            adapter_dropout = None
            print("EstimatorV3: Pure Transformer (No Adapter)")
