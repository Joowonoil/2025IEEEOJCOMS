...
from model.prompt_transformer_v4 import PromptTransformer, ConditionNetwork # Prompt-enabled v4 Transformer 모델 임포트

class PromptEstimator_v4(nn.Module): # PromptEstimator_v4 클래스 정의 (프롬프트 기반 채널 추정 모델)
    def __init__(self, conf_file): # 초기화 메서드
        super(PromptEstimator_v4, self).__init__() # 부모 클래스 초기화
        ...
        # Channel estimation network (Load pretrained Transformer with Prompt Learning) # 채널 추정 네트워크 (프롬프트 학습을 사용하는 사전 학습된 트랜스포머 로드)
        ch_cond_netw = ConditionNetwork(**self._conf['ch_estimation']['cond']) # ConditionNetwork 인스턴스 생성 (설정 파일에서 파라미터 로드)
        
        # Prompt learning 파라미터 추가
        prompt_config = self._conf['ch_estimation'].get('prompt', {}) # 설정 파일에서 prompt 설정 로드
        prompt_length = prompt_config.get('prompt_length', 5) # 프롬프트 토큰 수 (기본값 5)
        
        # Prompt-enabled Transformer 모델 인스턴스 생성
        self.ch_tf = PromptTransformer( # PromptTransformer 모델 인스턴스 생성
            **self._conf['ch_estimation']['transformer'], # 설정 파일에서 트랜스포머 파라미터 로드
            cond_net=ch_cond_netw, # ConditionNetwork 인스턴스 전달
            prompt_length=prompt_length # 프롬프트 토큰 수 전달
        )

        # Load pretrained weights (only if specified in config) # 사전 학습된 가중치 로드 (설정에 지정된 경우에만)
        if 'pretrained_model_name' in self._conf['training'] and self._conf['training']['pretrained_model_name']:
            ...
            # CUDA가 없는 환경에서도 모델을 로드할 수 있도록 CPU mapping 추가
            device = 'cuda' if torch.cuda.is_available() else 'cpu'
            full_state_dict = torch.load(pretrained_model_path, weights_only=False, map_location=device) # 모델 상태 사전 로드
            ...
            # 프롬프트 학습을 위해 사전 학습된 모델 가중치 로드 (프롬프트 토큰 제외)
            # 기존 transformer 모델 가중치 로드
            try:
                # prompt_tokens와 base_pe/prompt_pe를 제외한 나머지 파라미터만 로드
                filtered_state_dict = {k: v for k, v in transformer_state_dict_to_load.items()
                                      if '_prompt_tokens' not in k and 'base_pe' not in k and 'prompt_pe' not in k}
                missing_keys, unexpected_keys = self.ch_tf.load_state_dict(filtered_state_dict, strict=False) # 추출된 상태 사전 로드 (strict=False로 부분 로드 허용)
                ...
                print(f"Warning during model loading (expected for position encoding size mismatch): {e}") # 경고 메시지 출력 (위치 인코딩 크기 불일치는 정상)

            # Config에서 파라미터 동결 설정 확인 (try-except 밖에서 항상 실행)
            freeze_base_model = self._conf['ch_estimation']['prompt'].get('freeze_base_model', True)

            if freeze_base_model:
                # 기존 모델 파라미터를 frozen으로 설정 (프롬프트 토큰만 학습)
                for name, param in self.ch_tf.named_parameters():
                    if '_prompt_tokens' in name:
                        param.requires_grad = True # 프롬프트 토큰만 학습 가능
                    else:
                        param.requires_grad = False # 그 외 모든 파라미터 frozen (위치 인코딩 포함)
                print("Base model parameters frozen, only prompt tokens are trainable.") # 기본 모델 파라미터 frozen, 프롬프트 토큰만 학습 가능
            else:
                # 모든 파라미터 학습 가능 (프롬프트 + 전체 모델)
                for name, param in self.ch_tf.named_parameters():
                    param.requires_grad = True
                print("All parameters trainable (prompt + full model learning).") # 모든 파라미터 학습 가능
        else:
            print("No pretrained model specified - initializing from scratch for base model training") # 사전 학습된 모델이 지정되지 않음 - 베이스 모델 훈련을 위해 처음부터 초기화


        # Prompt Learning 기반 전이 학습
        # PromptEstimator_v4에서는 기본 Transformer 모델을 로드하고 frozen으로 설정
        # 프롬프트 토큰만 학습 가능하게 설정
        
        # 학습 가능한 파라미터 수 확인 및 디버깅 정보
        trainable_params = 0
        prompt_params = 0
        other_trainable_params = 0

        print("\n=== Prompt Learning Parameter Analysis (Option 1: Fixed Position Encoding) ===")
        for name, param in self.ch_tf.named_parameters():
            if param.requires_grad:
                ...
                if '_prompt_tokens' in name:
                    prompt_params += param_count
                    print(f"+ Prompt tokens: {name} - {param_count:,} params (trainable)")
                else:
                    other_trainable_params += param_count
                    print(f"+ Other trainable: {name} - {param_count:,} params")
            else:
                if 'prompt' in name or 'pe' in name.lower():
                    param_count = param.numel()
                    print(f"- Frozen: {name} - {param_count:,} params (frozen)")

        total_params = sum(p.numel() for p in self.ch_tf.parameters())

        print(f"\nParameter Summary:")
        print(f"- Prompt tokens (trainable): {prompt_params:,}")
        print(f"- Position encoding (frozen): included (buffer)")
        print(f"- Other trainable: {other_trainable_params:,}")
        print(f"- Total trainable: {trainable_params:,} ({trainable_params/total_params*100:.4f}%)")
        print(f"- Total parameters: {total_params:,}")
        print(f"- Efficiency vs LoRA: {98304/max(trainable_params,1):.1f}x fewer params")
        print("=" * 75 + "\n")
    ...
    def forward(self, rx_signal):  # rx_signal: (batch, 14, 3072, 2) # 순전파 메서드 (입력: 수신 신호)
        ...
        # Pass through the PromptTransformer # 프롬프트 트랜스포머 통과
        ch_est = self.ch_tf(ch_in) # (batch, channels, n_token * step_size) # 프롬프트 트랜스포머를 통해 채널 추정
...
if __name__ == "__main__": # 스크립트 직접 실행 시 실행되는 코드 블록
    ...
    estimator = PromptEstimator_v4(conf_file).cuda() # PromptEstimator_v4 인스턴스 생성 및 CUDA 디바이스로 이동
