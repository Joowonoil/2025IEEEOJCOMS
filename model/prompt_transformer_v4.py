...
class PromptTransformer(nn.Module): # Prompt-enabled Transformer 모델 클래스 정의
    def __init__(self, length, channels, num_layers, d_model, n_token, n_head, dim_feedforward, dropout, # 초기화 메서드
                 activation="relu", cond_net=None, prompt_length=5): # Prompt learning 파라미터 추가
        super(PromptTransformer, self).__init__() # 부모 클래스 초기화
        self._length = length # 입력 데이터 길이 저장
        ...
        # Prompt Learning 파라미터 추가 (Standard Prefix Tuning - Option 1)
        # Learnable: Prompt token contents only
        # Fixed: Position encoding (handled in PromptTransformerLayer)
        self._prompt_length = prompt_length # 프롬프트 토큰 수 저장
        self._prompt_tokens = Parameter(torch.randn(prompt_length, d_model), requires_grad=True) # 학습 가능한 프롬프트 토큰 (contents only)

        # Transformer layers # 트랜스포머 레이어 정의
        self._layers = nn.ModuleList() # ModuleList 생성
        for _ in range(self._num_layers): # 설정된 레이어 수만큼 반복
            layer = PromptTransformerLayer(self._d_model, self._n_token, n_head, dim_feedforward, dropout, # PromptTransformerLayer 인스턴스 생성
                                     activation="relu", cond_d_model=self._cond_d_model, # 활성화 함수, ConditionNetwork 차원 및 토큰 수 전달
                                     cond_n_token=self._cond_n_token, prompt_length=prompt_length
                                    )
            self._layers.append(layer) # 생성된 레이어를 ModuleList에 추가

        self._embedding = Parameter(torch.empty((self._n_token, self._d_model), requires_grad=False)) # 임베딩 파라미터 정의 (frozen for prompt learning)
    ...
    def _reset_parameters(self): # 파라미터 초기화 메서드
        ...
        # Prompt tokens를 base embedding 분포에 맞춰 초기화
        # Position encoding은 PromptTransformerLayer에서 fixed sinusoidal로 처리됨
        with torch.no_grad():
            if hasattr(self, '_embedding') and self._embedding.numel() > 0:
                # Base embedding의 통계를 따라 초기화
                mean = self._embedding.mean().item()
                std = self._embedding.std().item()
                nn.init.normal_(self._prompt_tokens, mean=mean, std=std)
            else:
                # Fallback 초기화
                nn.init.normal_(self._prompt_tokens, std=0.02)
    ...
    def forward(self, cond): # 순전파 메서드
        ...
        # Prompt tokens (contents only - position encoding은 PromptTransformerLayer에서 추가됨)
        prompt_tokens = self._prompt_tokens[:, None, :].repeat(1, n_batch, 1) # 프롬프트 토큰을 배치 크기만큼 반복

        # 기존 임베딩과 프롬프트 토큰 결합
        #x = repeat(self._embedding, 't d -> t b d', b=n_batch) # 임베딩 반복 (주석 처리됨)
        base_embedding = self._embedding[:, None, :].repeat(1, n_batch, 1) # 임베딩을 배치 크기만큼 반복
        x = torch.cat([prompt_tokens, base_embedding], dim=0) # 프롬프트 토큰을 앞에 추가
        for i, layer in enumerate(self._layers): # 각 트랜스포머 레이어를 순회
            x = layer(input=x, cond=c) # 트랜스포머 레이어 순전파 (여기서 fixed sinusoidal PE 추가됨)

        # 프롬프트 토큰 제거하고 원래 시퀀스만 사용
        x_without_prompt = x[self._prompt_length:] # 프롬프트 토큰 제거

        #y = rearrange(self._linear(x), 't b (c d) -> b c (t d)', c=self._channels) # rearrange 연산 (주석 처리됨)
        y = self._linear(x_without_prompt) # 최종 Linear 레이어 순전파 (프롬프트 토큰 제외)
...
class PromptTransformerLayer(nn.Module): # PromptTransformerLayer 클래스 정의 (프롬프트 지원 트랜스포머의 단일 레이어)
    def __init__(self, d_model, n_token, n_head, dim_feedforward, dropout, activation="relu", # 초기화 메서드
                 cond_d_model=None, cond_n_token=None, prompt_length=5): # Prompt learning 파라미터 추가
        super(PromptTransformerLayer, self).__init__() # 부모 클래스 초기화
        self._d_model = d_model # 모델 차원 저장
        self._n_token = n_token # 토큰 수 저장
        self._n_head = n_head # 어텐션 헤드 수 저장
        self._dim_feedforward = dim_feedforward # 피드포워드 네트워크 차원 저장
        ...
        # Positional encoding and mask # 위치 인코딩 및 마스크 (프롬프트와 base 토큰 분리)
        # Base tokens용 위치 인코딩 (원래 시퀀스)
        base_pe = positional_encoding_sine(n_pos=n_token, d_model=d_model, max_n_pos=n_token,
                                          normalize=True, scale=None)
        self.register_buffer('base_pe', base_pe)

        # Prompt tokens용 위치 인코딩 (새로 추가되는 토큰들)
        prompt_pe = positional_encoding_sine(n_pos=prompt_length, d_model=d_model, max_n_pos=prompt_length,
                                            normalize=True, scale=None)
        self.register_buffer('prompt_pe', prompt_pe)

        # 전체 시퀀스용 위치 인코딩 (prompt + base 결합)
        combined_pe = torch.cat([prompt_pe, base_pe], dim=0)
        self.register_buffer('pe', combined_pe)
